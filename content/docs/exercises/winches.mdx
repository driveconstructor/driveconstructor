...
Design the drive system for lowest cost. Consider topologies with gearbox and direct drive.

import Image from "next/image";

import W1Image from "./images/W1.png";

<img src={W1Image.src} width="60%" height="60%" />
...
<img src={W2Image.src} width="25%" height="25%" />


## W-3. Inertia in the winch drive train

The trawler is going forward dragging a fish net behind. The net is connected by a wire to the winch. The winch drum is being held at zero speed by torque control. Rated torque is applied. Sometimes the net may get stuck and the wire collapse.
...
* Force on the wire (and torque) increases by 20% right after the net stuck and them increases by 10% for every 0.1% of extension of the wire.

Design drive train so that the wire does not collapse. To find the answer try to dimension the drive train using several gearbox and machine combinations. Check if the wire would collapse in each design variant.