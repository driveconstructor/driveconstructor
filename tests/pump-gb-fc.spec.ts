...
test.beforeEach(async ({ page }) => {
  ...
  await expect(page.getByRole('main')).toContainText('Drive train with speed gearing');
});
...
test("Defaults", async ({ page }) => {
  ...
  await expect(page.getByTestId('gearbox[0].numberOfStages')).toContainText('1');
  await expect(page.getByTestId('gearbox[0].stage1Ratio')).toContainText('3');
  await expect(page.getByTestId('gearbox[0].stage1Type')).toContainText('helical');
  await expect(page.getByTestId('gearbox[0].gearRatio')).toContainText('3');
  await expect(page.getByTestId('gearbox[0].efficiency100')).toContainText('99');
  ...
  await expect(page.getByTestId('emachine[0].designation')).toContainText('IM-250-LV-400-SH355-ACS-IP2x-CI-600-B3-IE4');
  await expect(page.getByTestId('emachine[1].designation')).toContainText('IM-250-LV-400-SH400-ACS-IP2x-CI-500-B3-IE4');
  await page.getByTestId('emachine[0].<selected>').check();
  await expect(page.getByTestId('fconverter[0].designation')).toContainText('2Q-2L-400-200-IP2x-AC-W-6p');
  await expect(page.getByTestId('fconverter[1].designation')).toContainText('2Q-2L-400-200-IP2x-AC-F-6p');
  });
